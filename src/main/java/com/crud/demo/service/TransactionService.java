			...
			for (Orders orderbuy : buyorders)// buyers loop
			{
				for (Orders ordersell : sellorders)// sellers loop
				{
					...
					if (orderbuyCoinName.equals(ordersellCoinName)) 
					{
						...
						//getting buyers both wallets
						User buyerUser = orderbuy.getUser();
						Set<UserWallet> userWallets = buyerUser.getUserWallet();
						UserWallet userBuyerWalletFiate = null;
						UserWallet userBuyerWalletNew = null;
						for (UserWallet userBuyerWallet : userWallets) {
							if (("fiate").equals(userBuyerWallet.getWalletType())) {
								userBuyerWalletFiate = userBuyerWallet;
							} else if (orderbuy.getCoinName().equals(userBuyerWallet.getWalletType())) {
								userBuyerWalletNew = userBuyerWallet;
							}

						}
						// getting Seller both wallets
						...
						{
							LOGGER.info("Buyer wallet updation starts if condition ");
							
							userBuyerWalletFiate.setBalance(userBuyerWalletFiate.getShadowBalance());
							userBuyerWalletNew.setBalance(orderbuy.getCoinQuantity());
							userBuyerWalletNew.setShadowBalance(userBuyerWalletNew.getShadowBalance() + orderbuy.getCoinQuantity());
							userWalletJpaRepository.save(userBuyerWalletFiate);
							userWalletJpaRepository.save(userBuyerWalletNew);
							
							
							LOGGER.info("Seller wallet updation starts if condition");
							
							userSellerWalletFiate.setBalance(userSellerWalletFiate.getShadowBalance());
							userSellerWalletNew.setBalance(ordersell.getCoinQuantity());
							userSellerWalletNew.setShadowBalance(userSellerWalletNew.getShadowBalance());
							userWalletJpaRepository.save(userBuyerWalletFiate);
							userWalletJpaRepository.save(userBuyerWalletNew);
						}
						else
						{ LOGGER.info("Buyer wallet updation starts else condition ");
						Integer remainingQuantityNeed = orderbuy.getCoinQuantity()-coinManagement.getInitialSupply();
						  LOGGER.info("Seller wallet updation starts else condition");
				...
				if (coinSetAvailableInAdmin.contains(orderbuy.getCoinName())) {
					...
					if (orderbuy.getQuoteValue() >= coinManagement.getPrice()
						...
						if (coinManagement.getInitialSupply() >= orderbuy.getCoinQuantity()
							...
							userWalletNew
									.setShadowBalance(userWalletNew.getShadowBalance() + orderbuy.getCoinQuantity());
							...
							userWalletNew.setShadowBalance(
									userWalletNew.getShadowBalance() + coinManagement.getInitialSupply());
							userWalletJpaRepository.save(userWalletFiate);
							userWalletJpaRepository.save(userWalletNew);
						}
						...
						System.out
								.println(":::coinManagement.getInitialSupply()::" + coinManagement.getInitialSupply());
						...
						if (coinManagement.getInitialSupply() >= buyOrderQuantity) {
							...
							coinManagement.setINRConvergent(inrConvergentOfDemandingCoin+coinManagement.getINRConvergent());
							...
							Integer updatedFee = (((buyOrderQuantity - remainingCoinNeed) * coinManagement.getPrice())
									* 2) / 100;
